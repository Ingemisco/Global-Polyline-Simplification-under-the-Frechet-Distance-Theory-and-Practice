...
using Simplification::simplification_bc_euclidean_explicit;
using Simplification::simplification_bc_chebyshev_explicit;
using Simplification::simplification_bc_manhattan_explicit;
using Simplification::simplification_bc_euclidean_semiexplicit;
using Simplification::simplification_klw_euclidean;
using Simplification::simplification_klw_chebyshev;
using Simplification::simplification_klw_manhattan;
using Simplification::simplification_klw_euclidean_semiexplicit;
using Simplification::simplification_klw_euclidean_implicit;
...
static inline void compare_semiexplicit(Polyline const &polyline, float epsilon) {
	size_t const euclidean_size_bc = simplification_bc_euclidean_explicit(polyline, epsilon, config)->size();

	float const epsilon2 = epsilon * epsilon;
	size_t const euclidean_size_klw_semi = simplification_klw_euclidean_semiexplicit(polyline, epsilon2, config)->size();
	size_t const euclidean_size_bc_semi = simplification_bc_euclidean_semiexplicit(polyline, epsilon2, config)->size();


	BOOST_CHECK_EQUAL(euclidean_size_klw_semi, euclidean_size_bc_semi);
	BOOST_CHECK_EQUAL(euclidean_size_bc_semi, euclidean_size_bc);
}
...
static inline void compare_sizes(Polyline const &polyline, float epsilon) {
	size_t const manhattan_size_klw    = simplification_klw_manhattan(polyline, epsilon, config)->size();
	size_t const manhattan_size_bc = simplification_bc_manhattan_explicit(polyline, epsilon, config)->size();
	size_t const chebyshev_size_klw    = simplification_klw_chebyshev(polyline, epsilon, config)->size();
	size_t const chebyshev_size_bc = simplification_bc_chebyshev_explicit(polyline, epsilon, config)->size();
	size_t const euclidean_size_klw    = simplification_klw_euclidean(polyline, epsilon, config)->size();
	size_t const euclidean_size_bc = simplification_bc_euclidean_explicit(polyline, epsilon, config)->size();
	size_t const euclidean_size_implicit = simplification_klw_euclidean_implicit(polyline, epsilon, config)->size();

	size_t const euclidean_size_local    = Simplification::simplification_imai_iri_euclidean(polyline, epsilon, config)->size();
  
	BOOST_CHECK_EQUAL(euclidean_size_bc, euclidean_size_klw);
	BOOST_CHECK_EQUAL(euclidean_size_bc, euclidean_size_implicit);
	BOOST_CHECK_EQUAL(manhattan_size_bc, manhattan_size_klw);
	BOOST_CHECK_EQUAL(chebyshev_size_bc, chebyshev_size_klw);
  
	BOOST_CHECK_LE(euclidean_size_bc, manhattan_size_bc);
